...
Use the {{APPTICS_MCP_NAME}} MCP server. Fetch all crashes and crash details for {{APP_DISPLAY_NAME}} iOS app, for the version number {{CRASH_VERSIONS}} from the previous 24 hours. Save the crash details to 'AppticsCrash_<number>.crash' text files, in AppticsCrashLogs.in 'AppticsCrashLogs/' directory.
